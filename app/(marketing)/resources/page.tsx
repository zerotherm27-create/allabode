        ...
        <Container>
          ...
          <div className="mt-12 flex flex-wrap justify-center gap-3">
            {categories.map((cat) => (
              ...
              >
                <Icon name={cat.icon} size={18} className="text-gold-ink" />
                {cat.label}
              </a>
            ))}
          </div>
        ...
        <Container>
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {articles.map((group) => (
              ...
              >
                <div className="flex items-center gap-2.5">
                  <Icon name={group.icon} size={20} className="text-gold-ink" />
                  <p className="label-caps text-gold-ink">{group.category}</p>
                </div>
                <ul className="mt-6 space-y-4">
                  {group.items.map((item) => (
                    <li key={item.title}>
                      <Link
                        href={item.href}
                        className="group flex items-start gap-2 text-sm font-medium text-navy hover:text-gold-ink"
                      >
                        <Icon
                          name="arrow_forward"
                          size={16}
                          className="mt-0.5 shrink-0 text-gold-ink transition-transform group-hover:translate-x-0.5"
                        />
                        {item.title}
                      </Link>
                    </li>
            ...
            <div className="border border-dashed border-line bg-surface p-7">
              <div className="flex items-center gap-2.5">
                <Icon name="more_horiz" size={20} className="text-slate" />
