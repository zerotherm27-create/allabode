...
import { TenancyCountersignForm } from "@/components/admin/tenancy-countersign-form";
import { TenancyInventoryForm } from "@/components/admin/tenancy-inventory-form";
import { TenancyTermsForm, type TenancyTermsInitial, type UnitOption } from "@/components/admin/tenancy-terms-form";
import {
  sendTenancyTenantLink, sendTenancyLandlordLink, updateTenancyTerms, updateTenancyInventory,
...
type TenancyAgreement = {
  ...
  property_details: { buildingName?: string; floorUnit?: string; address?: string } | null;
  ...
  tenant_details: { name?: string; idNumber?: string; address?: string; contact?: string; email?: string } | null;
...
function toTermsInitial(a: TenancyAgreement): TenancyTermsInitial {
  ...
  return {
    ...
    floorUnit: pd.floorUnit ?? "",
    propertyAddress: pd.address ?? "",
    leaseMonths: a.lease_months !== null ? String(a.lease_months) : "12",
    ...
    advanceDepositAmount: a.advance_deposit_amount !== null ? String(a.advance_deposit_amount) : "",
    advanceDepositWords: a.advance_deposit_words ?? "",
    depositAmount: a.deposit_amount !== null ? String(a.deposit_amount) : "",
...
export default async function AdminTenancyContractDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const supabase = await createClient();

  const [{ data }, { data: { user } }] = await Promise.all([
    supabase.from("tenancy_agreements").select("*").eq("id", id).maybeSingle(),
  ...
  const units: UnitOption[] = ((unitsResult.data ?? []) as unknown as UnitRow[]).map((u) => ({
    id: u.id,
  ...
  const ld = a.landlord_details ?? {};
  const pd = a.property_details ?? {};
  const doSendTenantLink = sendTenancyTenantLink.bind(null, id);
  const doSendLandlordLink = sendTenancyLandlordLink.bind(null, id);
  const doUpdateTerms = updateTenancyTerms.bind(null, id);
  const doUpdateInventory = updateTenancyInventory.bind(null, id);
  const doFinalize = finalizeTenancyAgreement.bind(null, id);
  const doVoid = voidTenancyAgreement.bind(null, id);
  const doDelete = deleteTenancyAgreement.bind(null, id);
  const tenantLink = `${getPublicSiteUrl()}/sign/tenancy/${a.access_token}`;
  const landlordLink = a.landlord_access_token ? `${getPublicSiteUrl()}/sign/tenancy/landlord/${a.landlord_access_token}` : null;
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="label-caps text-gold">Tenancy Agreement</p>
          <h1 className="mt-1 font-display text-2xl font-bold text-navy">{td.name || a.tenant_name_hint || a.tenant_email}</h1>
          <p className="mt-1 text-sm text-slate">
            {a.tenant_email}{pd.buildingName ? ` · ${pd.buildingName}${pd.floorUnit ? ` ${pd.floorUnit}` : ""}` : ""}
          </p>
        </div>
      ...
      <div className="mt-6 rounded-lg border border-line bg-surface p-5">
        <h2 className="mb-3 font-display text-sm font-semibold text-navy">Lease terms</h2>
        <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
          {[
            ["Landlord", ld.name],
            ["Landlord email", a.landlord_email],
            ["Property", pd.buildingName],
            ["Floor / unit", pd.floorUnit],
            ["Lease period", a.lease_months ? `${a.lease_months} month/s` : null],
            ["Start — end", a.lease_start_date ? `${a.lease_start_date} — ${a.lease_end_date ?? "?"}` : null],
            ["Monthly rent", a.rent_amount !== null ? `₱${Number(a.rent_amount).toLocaleString("en-PH")}` : null],
            ["Rent due day", a.rent_due_day ? `Every ${a.rent_due_day}` : null],
            ["Advance + deposit", a.advance_deposit_amount !== null ? `₱${Number(a.advance_deposit_amount).toLocaleString("en-PH")}` : null],
            ["Security deposit", a.deposit_amount !== null ? `₱${Number(a.deposit_amount).toLocaleString("en-PH")}` : null],
            ["Linked unit", a.unit_id ? "Yes — lease record auto-created on completion" : "No — create the lease manually"],
          ].map(([k, v]) => (
        ...
        <div className="mt-6 rounded-lg border border-line bg-surface p-5">
          ...
          <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
            {[
              ...
              ["Occupants", (a.occupants ?? []).filter(Boolean).join(", ") || null],
      ...
      {a.status === "tenant_signed" && !awaitingFinalize && (
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          ...
          {isSignatory ? (
            <TenancyCountersignForm agreementId={id} />
          ) : (
      ...
      {a.status === "completed" && (
        <div className="mt-6 rounded-lg border border-available/30 bg-available/5 p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-4 text-sm">
            ...
            {a.linked_lease_id && (
              <Link href={`/admin/leases/${a.linked_lease_id}/edit`} className="text-navy-700 underline">Lease record</Link>
            )}
          </div>
          {!a.linked_lease_id && (
            <p className="mt-3 rounded-md bg-gold/10 px-3 py-2 text-xs text-gold-bright">
              No lease record was created{a.unit_id ? "" : " because no managed unit was linked"} —{" "}
              <Link href="/admin/leases/new" className="underline">create the lease manually</Link> so invoicing can
              pick it up.
            </p>
          )}
        </div>
