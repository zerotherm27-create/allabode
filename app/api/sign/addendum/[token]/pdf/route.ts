...
/**
 * Token-gated download for the fully-signed addendum PDF. One route serves
 * both parties: the token is tried as the tenant-side credential first, then
 * as the landlord's (each SECURITY DEFINER RPC only matches its own token
 * column). Only serves the file once status = 'completed'.
 */
export async function GET(_req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  const record = addendum as { status: string; pdf_path: string | null };
  ...
  return new NextResponse(await pdf.arrayBuffer(), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="addendum.pdf"`,
