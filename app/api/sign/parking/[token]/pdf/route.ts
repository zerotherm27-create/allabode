...
/**
 * Token-gated download for the fully-signed parking agreement PDF. One route
 * serves both parties: the token is tried as the tenant's credential first,
 * then as the landlord's (each SECURITY DEFINER RPC only matches its own
 * token column). Only serves the file once status = 'completed'.
 */
export async function GET(_req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const supabase = await createClient();

  let { data: agreement } = await supabase.rpc("get_parking_agreement_by_token", { p_token: token });
  if (!agreement) {
    ({ data: agreement } = await supabase.rpc("get_parking_agreement_by_landlord_token", { p_token: token }));
  }
  ...
  return new NextResponse(await pdf.arrayBuffer(), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="parking-rental-agreement.pdf"`,
