...
/**
 * Token-gated download for the fully-signed tenancy agreement PDF. One route
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  let { data: agreement } = await supabase.rpc("get_tenancy_agreement_by_token", { p_token: token });
  if (!agreement) {
    ({ data: agreement } = await supabase.rpc("get_tenancy_agreement_by_landlord_token", { p_token: token }));
  }
