...
export default function PayOwnerInvoicePage() {
  ...
  async function startPayment() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/payments/create-owner-invoice", {
        method:  "POST",
  ...
  return (
    <div className="flex min-h-dvh flex-col items-center justify-center bg-surface-gray px-4">
      <Reveal as="div" className="w-full max-w-sm rounded-xl border border-line bg-surface p-8 shadow-sm text-center">
        ...
        <Link href={`/dashboard/owner/invoices/${id}`} className="mt-4 block text-sm text-slate hover:text-navy">
          ← Cancel and go back
        </Link>
      </Reveal>
