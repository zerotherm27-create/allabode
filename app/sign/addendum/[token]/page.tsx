...
import { loadAddendum } from "@/app/sign/addendum-actions";
import { AddendumWizard } from "./wizard";

export const metadata = { title: "Sign your addendum", robots: { index: false } };

export default async function SignAddendumPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const addendum = await loadAddendum(token);
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-navy px-4 py-10">
      <AddendumWizard token={token} initial={addendum} />
    </div>
  );
}
