...
import {
  saveAddendumDraft, createAddendumIdUploadTicket, createAddendumPartyIdUploadTicket,
  confirmAddendumIdUpload, submitAddendumTenantSignature,
  type AddendumRecord,
} from "@/app/sign/addendum-actions";
import { createClient } from "@/lib/supabase/client";
import { AGREEMENTS_BUCKET } from "@/lib/storage";
import {
  SIGNING_ID_TYPES, tenantContactPrefill,
  normalizeOccupantIdUploads, type OccupantIdUpload,
} from "@/lib/signing/form-helpers";
import { addendumRoles } from "@/lib/pm/addendum-clauses";
import { FullAddendumPreview } from "./full-addendum-preview";
...
/** People this addendum adds or substitutes — each needs a copy of their ID. */
function newPartyNames(record: AddendumRecord): string[] {
  return (record.party_changes ?? [])
    .filter((c) => c.action === "add" || c.action === "substitute")
    .map((c) => c.name?.trim())
    .filter((n): n is string => !!n);
}

export function AddendumWizard({ token, initial }: { token: string; initial: AddendumRecord }) {
  const [step, setStep] = useState(initial.status === "tenant_signed" || initial.status === "completed" ? DONE_STEP : 1);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");

  const [tenant, setTenant] = useState<TenantForm>(tenantContactPrefill(initial));
  ...
  const partyNames = newPartyNames(initial);
  const [partyIdUploads, setPartyIdUploads] = useState<OccupantIdUpload[]>(() =>
    normalizeOccupantIdUploads((initial.tenant_details as { additionalOccupantIds?: unknown })?.additionalOccupantIds)
  );
  const [partyIdUploading, setPartyIdUploading] = useState<number | null>(null);

  const [typedName, setTypedName] = useState("");
  const [agreeChecked, setAgreeChecked] = useState(false);
  const padRef = useRef<SignatureInputHandle>(null);

  const roles = addendumRoles(initial.parent_type);
  const parentTitle = initial.parent_snapshot?.contractTitle || "agreement";

  async function persist() {
    setSaving(true);
    try {
      const { error: err } = await saveAddendumDraft(token, {
        tenantDetails: { ...tenant, additionalOccupantIds: partyIdUploads },
        tenantIdType: idType,
  ...
  async function next() {
    ...
    if (step === 1) {
      ...
      const missing = partyNames.filter(
        (_, i) => !partyIdUploads.some((u) => u.occupantIndex === i && u.path)
      );
      if (missing.length > 0) {
        setError(`A valid government ID image is required for: ${missing.join(", ")}.`);
        return;
      }
    }
  ...
  async function onIdFileChange(file: File) {
    ...
    setError("");
    try {
      const ticket = await createAddendumIdUploadTicket(token, file.name, file.size, file.type);
      ...
      const { error: confirmErr } = await confirmAddendumIdUpload(token, ticket.path);
      if (confirmErr) {
        setError(confirmErr);
        return;
      }
  ...
  async function onPartyIdFileChange(index: number, name: string, file: File) {
    setPartyIdUploading(index);
    setError("");
    try {
      const ticket = await createAddendumPartyIdUploadTicket(token, file.name, file.size, file.type);
      if (ticket.error || !ticket.signedUrl || !ticket.uploadToken || !ticket.path) {
        setError(ticket.error || "Could not prepare upload.");
        return;
      }
      ...
      // The path rides inside tenant_details, persisted on the next save —
      // same as the tenancy and parking flows' occupant IDs.
      setPartyIdUploads((prev) => [
        ...prev.filter((u) => u.occupantIndex !== index),
        { occupantIndex: index, occupantName: name, path: ticket.path!, fileName: file.name, uploadedAt: new Date().toISOString() },
      ]);
    } catch {
      setError("Upload failed — please check your connection and try again.");
    } finally {
      setPartyIdUploading(null);
    }
  }
  ...
  async function sign() {
    ...
    if (!agreeChecked) {
      setError("Please confirm you have read and agree to the Addendum.");
      return;
    }
    setSaving(true);
    try {
      ...
      const { error: err } = await submitAddendumTenantSignature(token, { typedName, signatureDataUrl: dataUrl });
      if (err) {
        setError(err);
        return;
      }
  ...
  return (
    <div className="w-full max-w-2xl rounded-lg border border-line bg-surface p-6 sm:p-8">
      ...
      {step === 1 && (
        <StepShell title="Your details">
          <p className="rounded-md bg-surface-gray px-4 py-3 text-xs text-slate">
            This Addendum amends your {parentTitle}
          ...
          {partyNames.map((name, index) => {
            const uploaded = partyIdUploads.some((u) => u.occupantIndex === index && u.path);
            const uploading = partyIdUploading === index;
            return (
              <div key={`party-id-${index}`} className="border-t border-line pt-4">
                <Field label={`${name}'s ID`} required hint="Required for each person this addendum adds">
                  <FileUploadButton
                    accept="image/jpeg,image/png"
                    disabled={uploading}
                    onFile={(file) => onPartyIdFileChange(index, name, file)}
                ...
                {!uploaded && !uploading && (
                  <p className="mt-2 flex items-center gap-1.5 text-xs text-slate">
                    <Icon name="info" size={16} /> Required — you cannot continue until this ID image is uploaded.
                  </p>
                )}
              </div>
            );
      ...
      {step === 2 && (
        <StepShell title="Review your addendum">
          <p className="text-xs text-slate">
            Please read the full addendum below before signing. Everything not amended here stays exactly as it is in
      ...
      {step === 3 && (
        <StepShell title="Sign your addendum">
          ...
          <label className="flex items-start gap-3 text-sm text-slate">
            ...
            <span>I have read and agree to this Addendum, and I am electronically signing in accordance with R.A. 8792.</span>
          </label>
        </StepShell>
      )}
      ...
      {step === DONE_STEP && (
        <div className="flex flex-col items-center py-8 text-center">
          ...
          <h3 className="mt-5 font-display text-2xl font-bold text-navy">
            {initial.status === "completed" ? "Addendum fully executed" : "Thank you for signing"}
          </h3>
          <p className="mt-3 max-w-md text-slate">
            {initial.status === "completed"
              ? "Both parties have signed. Your copy is ready to download below."
              : `We've received your signature. The ${roles.principal.toLowerCase()} will sign next, and we'll email you once the addendum is fully executed.`}
          </p>
          {initial.status === "completed" && (
            <a
              href={`/api/sign/addendum/${token}/pdf`}
            ...
            >
              <Icon name="download" size={20} /> Download signed addendum
            </a>
          )}
        </div>
      )}
