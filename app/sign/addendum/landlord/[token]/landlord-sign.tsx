...
import {
  createAddendumLandlordIdUploadTicket, confirmAddendumLandlordIdUpload, submitAddendumLandlordSignature,
  type AddendumRecord,
...
import { FullAddendumPreview } from "../../[token]/full-addendum-preview";

const inputCls = "h-9";

export function AddendumLandlordSign({ token, initial }: { token: string; initial: AddendumRecord }) {
  ...
  async function onIdFileChange(file: File) {
    ...
    setIdUploading(true);
    try {
      const ticket = await createAddendumLandlordIdUploadTicket(token, file.name, file.size, file.type);
      ...
      const { error: confirmErr } = await confirmAddendumLandlordIdUpload(token, {
  ...
  async function sign() {
    ...
    try {
      ...
      const result = await submitAddendumLandlordSignature(token, { typedName, signatureDataUrl: dataUrl });
      if (result.error) {
  ...
  return (
    <div className="w-full max-w-2xl rounded-lg border border-line bg-surface p-6 sm:p-8">
      <div className="mb-6 flex flex-col items-center gap-2">
        ...
        <p className="label-caps text-gold">Addendum — {roles.principal} signature</p>
      </div>
      ...
      {done ? (
        <div className="flex flex-col items-center py-8 text-center">
          ...
          <h3 className="mt-5 font-display text-2xl font-bold text-navy">
            {done.completed ? "Addendum fully executed" : "Signature received"}
          </h3>
        ...
        <div className="flex flex-col gap-6">
          <p className="rounded-md bg-surface-gray px-4 py-3 text-xs text-slate">
            {(initial.tenant_details?.name || `The ${roles.counterparty.toLowerCase()}`)} signed this addendum to the {parentTitle}
            ...
            Please review the full addendum, verify your ID, and sign below.
          </p>
          ...
          <div className="border-t border-line pt-5">
            ...
            <p className="mb-3 text-xs text-slate">
              Required before you can sign — your ID appears on the addendum&#x2019;s &quot;Copy of Valid IDs&quot; page.
            </p>
            ...
            <Field label="Upload ID image" required hint="JPG or PNG image, up to 10 MB — enter the ID number and issue date first">
              <FileUploadButton
            ...
            {!idUploaded && !idUploading && (
              <p className="flex items-center gap-1.5 text-xs text-slate"><Icon name="info" size={16} /> Required — you cannot sign until your ID image is uploaded.</p>
            )}
          </div>

          <div className="border-t border-line pt-5">
            <h3 className="mb-3 text-sm font-semibold text-navy">Sign as {roles.principal}</h3>
          ...
          <button
            ...
            onClick={sign}
            disabled={saving}
            className="inline-flex items-center justify-center gap-2 self-end rounded-md bg-navy px-6 py-3 text-sm font-semibold text-white hover:bg-navy-800 disabled:opacity-50"
          >
