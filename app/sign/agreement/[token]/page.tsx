...
import { loadAgreement } from "@/app/sign/agreement-actions";
import { AgreementWizard } from "./wizard";

export const metadata = { title: "Sign your agreement", robots: { index: false } };
...
export default async function SignAgreementPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-navy px-4 py-10">
      <AgreementWizard token={token} initial={agreement} />
    </div>
  );
}
