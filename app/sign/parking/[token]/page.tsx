...
import { loadParkingAgreement } from "@/app/sign/parking-actions";
import { ParkingWizard } from "./wizard";

export const metadata = { title: "Sign your parking agreement", robots: { index: false } };

export default async function SignParkingPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const agreement = await loadParkingAgreement(token);
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-navy px-4 py-10">
      <ParkingWizard token={token} initial={agreement} />
    </div>
  );
}
