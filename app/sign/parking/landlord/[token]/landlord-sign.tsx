...
export function ParkingLandlordSign({ token, initial }: { token: string; initial: ParkingAgreementRecord }) {
  ...
  async function onIdFileChange(file: File) {
    ...
    setIdUploading(true);
    try {
      const ticket = await createParkingLandlordIdUploadTicket(token, file.name, file.size, file.type);
      ...
      const { error: confirmErr } = await confirmParkingLandlordIdUpload(token, {
  ...
  async function sign() {
    ...
    try {
      ...
      const result = await submitParkingLandlordSignature(token, { typedName, signatureDataUrl: dataUrl });
      if (result.error) {
  ...
  return (
    <div className="w-full max-w-2xl rounded-lg border border-line bg-surface p-6 sm:p-8">
      <div className="mb-6 flex flex-col items-center gap-2">
        ...
        <p className="label-caps text-gold">Parking Space Rental Agreement — Landlord signature</p>
      </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="max-h-[55vh] overflow-y-auto rounded-md border border-line bg-surface-gray p-5">
            <FullParkingPreview
              ...
              vehicleDetails={initial.vehicle_details ?? {}}
            />
          </div>
