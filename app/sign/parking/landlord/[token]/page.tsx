...
import { loadParkingAgreementForLandlord } from "@/app/sign/parking-actions";
import { ParkingLandlordSign } from "./landlord-sign";

export const metadata = { title: "Sign the parking agreement", robots: { index: false } };

export default async function SignParkingLandlordPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const agreement = await loadParkingAgreementForLandlord(token);
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-navy px-4 py-10">
      <ParkingLandlordSign token={token} initial={agreement} />
    </div>
  );
}
