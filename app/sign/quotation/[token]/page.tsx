...
import { loadQuotation } from "@/app/sign/quotation-actions";
import { QuotationRecipientSign } from "./recipient-sign";

export const metadata = { title: "Review your quotation", robots: { index: false } };

export default async function SignQuotationPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const quotation = await loadQuotation(token);
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-navy px-4 py-10">
      <QuotationRecipientSign token={token} initial={quotation} />
    </div>
  );
}
