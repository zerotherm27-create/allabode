...
import { loadTenancyAgreement } from "@/app/sign/tenancy-actions";
import { TenancyWizard } from "./wizard";

export const metadata = { title: "Sign your tenancy agreement", robots: { index: false } };

export default async function SignTenancyPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const agreement = await loadTenancyAgreement(token);
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-navy px-4 py-10">
      <TenancyWizard token={token} initial={agreement} />
    </div>
  );
}
