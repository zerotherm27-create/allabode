...
export function TenancyWizard({ token, initial }: { token: string; initial: TenancyAgreementRecord }) {
  ...
  async function onIdFileChange(file: File) {
    ...
    setError("");
    try {
      const ticket = await createTenancyIdUploadTicket(token, file.name, file.size, file.type);
      ...
      const { error: confirmErr } = await confirmTenancyIdUpload(token, ticket.path);
      if (confirmErr) {
        setError(confirmErr);
        return;
      }
  ...
  async function sign() {
    ...
    try {
      ...
      const { error: err } = await submitTenantSignature(token, { typedName, signatureDataUrl: dataUrl });
      if (err) {
        setError(err);
        return;
      }
      ...
      {step === 1 && (
        <StepShell title="Your details">
          <p className="rounded-md bg-surface-gray px-4 py-3 text-xs text-slate">
            The lease terms (property, rent, deposits, and schedule) have been prepared by All Abode — you&apos;ll
          ...
          <div className="mt-2 border-t border-line pt-5">
            <h3 className="mb-1 text-sm font-semibold text-navy">Occupants</h3>
            <p className="mb-3 text-xs text-slate">
              List everyone who will live in the unit, including yourself (clause 5 of the agreement).
            </p>
            <div className="flex flex-col gap-2">
              {occupants.map((o, i) => (
                <Input
                  ...
                  placeholder={`Occupant ${i + 1}${i === 0 ? " (you)" : ""}`}
                  value={o}
