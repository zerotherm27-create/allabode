...
import { loadTenancyAgreementForLandlord } from "@/app/sign/tenancy-actions";
import { LandlordSign } from "./landlord-sign";

export const metadata = { title: "Sign the tenancy agreement", robots: { index: false } };

export default async function SignTenancyLandlordPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const agreement = await loadTenancyAgreementForLandlord(token);
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-navy px-4 py-10">
      <LandlordSign token={token} initial={agreement} />
    </div>
  );
}
