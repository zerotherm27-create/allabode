 ...
 * brand / remarks). Editable until the agreement completes; rows left blank
...
export function TenancyInventoryForm({
  ...
  function setRow(i: number, patch: Partial<InventoryRow>) {
  ...
  return (
    <details className="rounded-lg border border-line bg-surface p-5">
      <summary className="cursor-pointer font-display text-sm font-semibold text-navy">
        Inventory List (annex)
      </summary>
      <p className="mt-2 text-xs text-slate">
        Optional — printed as the INVENTORY LIST page of the agreement. Blank rows print as blank lines to be
        completed at turnover.
      </p>
      {warnTenantSigned && (
        <p className="mt-2 rounded-md bg-gold/10 px-3 py-2 text-xs text-gold-bright">
          The tenant has already signed — inventory changes still print in the final PDF, so only correct or
      ...
      <form action={action} className="mt-4">
        ...
        <div className="mt-4">
          <SubmitButton label="Save inventory" />
        </div>
      </form>
    </details>
  );
