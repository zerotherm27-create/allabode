 ...
 * page-render path. Returns real places only, deduped and distance-sorted,
 * capped to a reasonable count for a listing page.
 */
export async function getNearbyPois(lat: number, lon: number, radiusM = 1500): Promise<RawPoi[]> {
  ...
  const all = results.flat().sort((a, b) => a.distanceM - b.distanceM);
